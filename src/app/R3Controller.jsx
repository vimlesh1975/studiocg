...
export default function R3Controller() {
    ...
    const [yPosition, setYPosition] = useState(0.00);
    ...
    return (<div>
        <div style={{ display: 'flex' }}>
            ...
            <div style={{ border: '1px solid red', width: 400 }}>
                ...
                <div>
                    set Y Position <input type="Number" step={0.01} value={yPosition} onChange={async (e) => {
                        setYPosition(e.target.value);
                        const res = await fetch("/api/sendCommand", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ command: `scene "${selectedProject}/${selectedScene}" nodes set "RootNode" "Transform.Position.Y" "${e.target.value}"` })
                        })
