...
float3 GenerateTextureCoordinatesReflection(float3 reflection)
{
    float m = 2 * sqrt(pow(reflection.x, 2) + pow(reflection.y, 2) + pow(reflection.z, 2));
    float3 refCoord = reflection / m + 0.5;
    return refCoord;
}
...
float3 refCoord = GenerateTextureCoordinatesReflection(input.Reflection);
float4 refTexPixel = ReflectionTexture.Sample(Sampler, refCoord.xy);
res.rgb = lerp(res.rgb, refTexPixel.rgb * refTexPixel.a, Reflectivity);
...
totalMaskAlpha += GetMaskAlpha(input.Position, ratioRender, 0, MaskFlags);
totalMaskAlpha += GetMaskAlpha(input.Position, ratioRender, 1, MaskFlags);
totalMaskAlpha += GetMaskAlpha(input.Position, ratioRender, 2, MaskFlags);
